...
router.post('/nextQuestion', async (req, res) => {
    ...
    try {
        ...
        const tc = await TestCompletion.findOne({ test_id, user_id, submitted: false }).sort({ 'createdon': -1 });
        const { branch_id, section_id } = await User.findOne({ _id: user_id });
        const { totalrounds } = await TestSubject.findOne({ test_id, branch_id, section_id });
        questionRound = {
        ...
        if (questionRound.currentRound > totalrounds) {
            ...
            const testsubjectcompletion = await TestSubject.findOneAndUpdate(filter, update, {
                new: true
            });
            return res.status(200).json({ nextQuestion, questionround: questionRound, completed: true });
        }
        nextQuestion = await Question.findOne({ _id: tc.question_id });
        res.status(200).json({ nextQuestion, questionround: questionRound, completed: false });
    } catch (error) {
        console.error(error.message);
        res.status(500).send('Server error');
    }
...
router.post('/baseQuestions/list', async (req, res) => {
    ...
    try {
        let base_question_ids = await BaseTest.find({ _id: base_id }).select('question_ids -_id');
        if (base_question_ids.length > 0)
            base_question_ids = base_question_ids[0]['question_ids'];
        const base_questions = await Question.find({ _id: { $in: base_question_ids } });
        ...
        if (testcompletion === undefined || testcompletion === null || testcompletion.length === 0) {
