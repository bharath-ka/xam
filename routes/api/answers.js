...
router.post('/basequestion/evaluate', async (req, res) => {
    const {
        user_id, qanswers, test_id, base_id, module_ids
    } = req.body;
    const eval = {
        'easy': 0,
        'medium': 0,
    ...
    try {
        ...
        const testcompletion = await TestCompletion.findOneAndUpdate(filter, update, {
            new: true
        });

        const chapter_ids = await Chapter.find({ module_id: { $in: module_ids } });
        const questions = await Question.find({ chapter_id: { $in: chapter_ids }, rank });
...
router.post('/question/evaluate', async (req, res) => {
    ...
    try {
        ...
        if (questionround.currentRound > totalrounds) {
            ...
            return res.status(200).json({ nextQuestion, questionround, completed: true });
        }
        const chapter_ids = await Chapter.find({ module_id: { $in: module_ids } });
        const questions = await Question.find({ chapter_id: { $in: chapter_ids }, rank });
